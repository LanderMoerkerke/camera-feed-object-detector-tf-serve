...
def freeze_graph(sess, output_graph):
    ...
    print("{} ops written to {}.".format(
        len(output_graph_def.node), output_graph))
...
def convert_to_original_size(box, size, original_size, is_letter_box_image):
    if is_letter_box_image:
        box = box.reshape(2, 2)
        box[0, :] = letter_box_pos_to_original_pos(
            box[0, :], size, original_size)
        box[1, :] = letter_box_pos_to_original_pos(
            box[1, :], size, original_size)
    else:
...
def letter_box_image(image: Image.Image, output_height: int, output_width: int, fill_value)-> np.ndarray:
    ...
    fit_image = np.asarray(image.resize(
        (fit_width, fit_height), resample=Image.BILINEAR))
    ...
    to_return[pad_top:pad_top+fit_height,
              pad_left:pad_left+fit_width] = fit_image
    return to_return
